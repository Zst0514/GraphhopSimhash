...
int optional_int_arg(int argc, char** argv, const std::string& name, int default_value) {
    for (int i = 1; i + 1 < argc; ++i) {
        if (argv[i] == name) {
...
int main(int argc, char** argv) {
    try {
        ...
        const bool progress_enabled =
            has_flag(argc, argv, "--progress")
            || (!has_flag(argc, argv, "--no-progress") && ghhw::stderr_is_tty());
        const int progress_ms = optional_int_arg(argc, argv, "--progress-ms", 250);
        ...
        ghhw::AnalogCamResult result = engine.run(trace, progress_enabled ? &progress : nullptr);
