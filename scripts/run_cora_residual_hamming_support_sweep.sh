...
OUT_DIR="${OUT_DIR:-${ROOT_DIR}/output/residual_reuse/cora_hamming_support_sweep}"
RUNS="${RUNS:-1}"
SEED="${SEED:-42}"

# Cora is fast enough to scan the radius-2 region more densely.
# Radius is the maximum accepted Hamming distance in CAM lookup.
RADIUS_VALUES=(${RADIUS_VALUES:-2})
THRESHOLDS=(${THRESHOLDS:-20 22 25 28 30 35})
SPLITS=(${SPLITS:-"5:4" "6:5" "5:3" "4:3"})

mkdir -p "${OUT_DIR}/logs"
cd "${ROOT_DIR}"

echo "[Sweep] dataset=cora output=${OUT_DIR}"
...
for radius in "${RADIUS_VALUES[@]}"; do
  for threshold in "${THRESHOLDS[@]}"; do
    for split in "${SPLITS[@]}"; do
      hard="${split%%:*}"
      soft="${split##*:}"
      log="${OUT_DIR}/logs/cora_r${radius}_t${threshold}_h${hard}_s${soft}_runs${RUNS}.log"
      echo
      echo "================================================================"
...
logs = sorted((out_dir / "logs").glob("cora_r*_t*_h*_s*_runs*.log"))
...
name_re = re.compile(r"cora_r(?P<radius>\d+)_t(?P<t>\d+)_h(?P<h>\d+)_s(?P<s>\d+)_runs(?P<runs>\d+)\.log$")
...
with compact.open("w", encoding="utf-8") as f:
    f.write("Cora/ST residual reuse Hamming/support sweep\n")
