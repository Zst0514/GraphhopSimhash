...
#   8 x 16-bit SimHash heads, R=2, T=30, TSER weights=3/1/1
...
BUDGET_PRIORITIES=(${BUDGET_PRIORITIES:-random degree tser low_unique})
...
case "${DATASET}" in
  cora)
    ...
    RESIDUAL_GATE_THRESHOLD="${RESIDUAL_GATE_THRESHOLD:-0.575}"
    RESIDUAL_POSITIVE_ERROR_MAX="${RESIDUAL_POSITIVE_ERROR_MAX:--1}"
    RESIDUAL_OFFLINE_NEGATIVE_ANCHORS_PER_NODE="${RESIDUAL_OFFLINE_NEGATIVE_ANCHORS_PER_NODE:-0}"
    RESIDUAL_NEGATIVE_GATE_WEIGHT="${RESIDUAL_NEGATIVE_GATE_WEIGHT:-0.0}"
    RESIDUAL_ACCEPT_LOSS_WEIGHT="${RESIDUAL_ACCEPT_LOSS_WEIGHT:-1.0}"
    RESIDUAL_GATE_SPARSITY_WEIGHT="${RESIDUAL_GATE_SPARSITY_WEIGHT:-0.0}"
...
"${PYTHON_BIN}" -m GraphhopSimhash \
  ...
  --precision_depth_budget_priorities "${BUDGET_PRIORITIES[@]}" \
