...
RUNS="${RUNS:-3}"
OUT_DIR="${OUT_DIR:-output/tser_reuse_sweep/cora}"
...
# 1) Baseline: hash reuse only, no TSER accept/reject gate.
run_case "00_no_score" \
  --experiment_suite single \
  --disable_score_gate

# 2) Degree-only gate: protects high-propagation nodes, ignores context and rare low-degree risk.
...
# 3) TSER component ablations at the same threshold.
...
# 4) No-propagation variants: test whether context/unique risk can work without degree.
...
# 5) Stronger TSER penalties, useful when accuracy matters more than reuse rate.
...
# 6) Threshold sweep for the recommended light TSER score.
run_case "12_tser_light_T30_w311" \
  --experiment_suite single \
  --enable_score_gate \
...
run_case "13_tser_light_T60_w311" \
  --experiment_suite single \
  --enable_score_gate \
...
run_case "14_tser_light_T90_w311" \
  --experiment_suite single \
  --enable_score_gate \
  --score_reuse_threshold 90 \
  --score_propagation_weight 3 \
  --score_graph_context_weight 1 \
  --score_low_unique_weight 1
